...
const DBHeader = () => {
  ...
  <p className='text-2xl text-green-600 font-semibold'>Welcome to Good Food
  {user?.name &&(
    <span className="block text-base text-orange-500">{`Hello ${user?.name}...!`}</span>
  )}
  </p>
  <div className='flex items-center justify-center gap-4'>
        ...
        <motion.div {...buttonClick} className="w-10 h-10 rounded-md cursor-pointer bg-lightOverlay backdrop-blur-md shadow-md flex items-center justify-center">
            <BsFillBellFill className='text-orange-400 text-xl'/>
        </motion.div>

        <div className='flex items-center justify-center gap-2'>
           <div className='w-10 h-10 rounded-md shadow-md cursor-pointer overflow-hidden'>
                <motion.img className='w-full h-full object-cover' src={user?.picture ? user?.picture : Avatar} whileTap={{scale:1.15}} referrerPolicy="no-referrer"/>
           </div>
           <motion.div {...buttonClick} className="w-10 h-10 rounded-md cursor-pointer bg-lightOverlay backdrop-blur-md shadow-md flex items-center justify-center">
